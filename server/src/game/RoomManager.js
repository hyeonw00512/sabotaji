...
export class RoomManager {
  ...
  listPublicRooms() {
    return [...this.rooms.values()].filter(room => room.settings.isPublic).map(room => ({ roomCode:room.id, hostNickname:room.players.find(player => player.id === room.hostId)?.nickname || '알 수 없음', playerCount:room.players.length, maxPlayers:room.settings.maxPlayers, spectatorCount:room.spectators.length, status:room.status, visibility:'PUBLIC', rounds:room.settings.rounds, turnSeconds:room.settings.turnSeconds, requiresPassword:Boolean(room.passwordHash) }));
  }
