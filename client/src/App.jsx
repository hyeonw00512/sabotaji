...
export function App() {
  ...
  if (!state) return <><StartScreen busy={busy} initialCode={initialCode} publicRooms={publicRooms} records={records} onRefresh={refreshRooms} onRefreshRecords={refreshRecords} onCreate={payload => enter('createRoom', payload)} onJoin={payload => enter('joinRoom', payload)} onPlatform={returnToPlatform}/>{toast && <div className="toast">{toast}</div>}</>;
