...
export function Lobby({ state, me, isSpectator, onReady, onStart, onSend, onLeave, onPlatform }) {
  ...
  return <main className="lobby-shell">
    <header className="topbar"><div><span className="eyebrow">대기실 · 광산 입구</span><h1>방 코드 <strong>{state.roomCode}</strong></h1></div><div className="topbar-actions"><button className="secondary small" onClick={copy}>{copied ? '복사됨 ✓' : '초대 링크 복사'}</button><button className="secondary small" onClick={onPlatform}>플랫폼</button><button className="danger small" onClick={onLeave}>나가기</button></div></header>
